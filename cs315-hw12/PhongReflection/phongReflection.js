...
function calculateReflectionAndSpec(){
  ...
  */
  ...
  point.copy(surfacePointBall.position.clone().add(sphereObj.position));
  line.copy(sun.position.clone().sub(point));
  normal.copy(point.clone().sub(sphereObj.position).normalize());
  if ( normal.dot(line) > 0 ) {
    nDotL2n.copy(normal.clone().multiplyScalar(normal.dot(line) * 2));
    ray.subVectors(nDotL2n, line);
    yellowConeObj.position.copy(point.clone().add(ray));
    ...
    if(params.lighting == 'diffuse'){
      ...
      var col = new THREE.Color();
        ...
        col = diffuseColor;
        var deg = normal.clone().angleTo(line.clone());
        surfacePointBall.material.color.copy(col);
        surfacePointBall.material.color.offsetHSL(0,0, -(deg*0.2));

    }else{
      var col = new THREE.Color('white');
      var e = new THREE.Vector3;
      e.copy(eye.position.clone().sub(point));

      var deg = ray.clone().angleTo(e.clone());

      surfacePointBall.material.color.copy(col);
      var shiny = params.shininess/10;
      if(shiny>1){

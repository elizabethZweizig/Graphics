...
function drawCubes() {
  ...
  */
  ...
  // BEGIN CHANGES
    .add(new TG.CheckerBoard().size(60,60).tint(0,0,1))
    .add(new TG.CheckerBoard().size(60,60).offset(20,20).tint(1,0,0))
    .add(new TG.CheckerBoard().size(60,60).offset(40,40).tint(0,1,0))
  // END CHANGES
  ...
  */
  ...
  // BEGIN CHANGES
    .add(new TG.Circle().position(256/2,256/2).radius(256/2).tint(1,0,0))
    .xor(new TG.Circle().position(256/2,256/2).radius(256/4).tint(1,1,1))
    .min(new TG.SinY().frequency(0.1).tint(-2,-1,-1))
  // END CHANGES
  ...
  */
  ...
  // BEGIN CHANGES
  tgTex4.add( new TG.LinearGradient().interpolation( 1 )
      .point( 0,    [ 1, 0, 0 ] )
      .point( 0.2,  [ 1, 1, 0 ] )
      .point( 0.4,  [ 0, 1, 0 ] )
      .point( 0.6,  [ 0, 1, 1 ] )
  ...
  */
  var yourTex = new TG.Texture(256, 256)
  // BEGIN CHANGES
  .add( new TG.SinX().frequency( 0.2 ) )
   .sub( new TG.Number().tint( 1.5, 2.5, 6 ) )
   .add( new TG.SinX().frequency( 0.003 ).tint( 1.5, 1, 1 ) )
   .set( new TG.Twirl().radius( 256 / 2 ).position( 256 / 2, 256 / 2 ).strength ( 1000 ) )
  // END CHANGES
